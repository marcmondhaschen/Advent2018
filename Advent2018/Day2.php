 ...
 * sure either. But now that so many people have chimneys, maybe he could sneak
 * in that way?" Another voice responds, "Actually, we've been working on a new
 ...
 * would take forever to search the warehouse for two similar box IDs..."
 ...
 * and three of them contain a letter which appears exactly three times. Multi-
 * plying these together produces a checksum of 4 * 3 = 12.
 *
 * What is the checksum for your list of box IDs?
 *
 */

$handle = @fopen("day2data", "r");
if ($handle) {
    $id_array = array();
    $ids_with_pairs = 0;
    $ids_with_triplets = 0;
    ...
    foreach($id_array as $id){
        $char_counts = array_count_values(str_split($id));
        if(array_search(2,$char_counts)){
            $ids_with_pairs++;
        }
        if(array_search(3,$char_counts)){
            $ids_with_triplets++;
        }
    }

    echo "Part 1 is : " . $ids_with_pairs*$ids_with_triplets . "\n";
}
...
$input = file_get_contents('day2data');
$boxIds = explode("\n", $input);
$possibleMatches = [];
